# 9 to 5 @ JMR — Race Plan 2026

**Date:** June 13, 2026 | 9:00am – 5:00pm
**Venue:** Jug Mountain Ranch, McCall ID
**Category:** Solo 50+
**Kit:** Lost River
**Registration:** Zone4 #7174873 | Contact: alex01phipps@gmail.com

---

## Athlete Profile
| | |
|---|---|
| FTP | 277W |
| LTHR | 172bpm |
| Weight | 160lbs |
| Bike | Trek Top Fuel 9.9 Gen 4 |
| Drivetrain | SRAM XO Transmission |
| Power Meter | Quarq (crank) |
| Head Unit | Garmin 540 |

---

## Course
- **Distance:** 9.64 miles per lap
- **Climbing:** 1,369ft per lap
- **Elevation:** 5,091ft – 5,663ft
- **Surface:** Doubletrack and singletrack, fast and flowy
- **Key feature:** Vandelay trail at end of lap — tight technical turns, added 2026. Rides easy fresh, consequential when fatigued.
- **Lap must finish before 5:00pm to count**

---

## Logistics

### Friday June 12
| Time | Action |
|------|--------|
| ~2:30pm | Arrive Donnelly RV Park (full hookups) |
| 5:00pm | Drive to JMR — packet pickup opens 5pm to dark |
| On arrival | Set up pit spot at start/finish area |
| Evening | Preview lap — focus on Vandelay entry and lines |
| Night | Back to Donnelly — real dinner, early bed |

### Saturday June 13
| Time | Action |
|------|--------|
| 6:00am | Wake up |
| 6:30am | Breakfast burrito (4 eggs, meat, rice, 2 tortillas) |
| 7:15am | Depart Donnelly (~30 min to venue) |
| 7:45am | Arrive JMR — final pit setup |
| 8:00–8:45am | Warmup: easy spin, 2–3 x 30sec openers at ~220W |
| 8:50am | Stage |
| 9:00am | Race start (no LeMans run) |
| 5:00pm | Race end — all laps completed by 5pm count |
| 5:30pm | Podium + raffle + dinner at start/finish |

### Venue Notes
- No trailers on site — day trip from Donnelly
- No campfires
- 200 entry cap — confirmed registered
- Dinner included (registered before June 7 ✓) — Renee included
- Pit zone is on course at start/finish

---

## Target: 8 Laps

### Lap Time Targets
| Lap | Target | Notes |
|-----|--------|-------|
| 1 | Controlled | No LeMans — controlled start from gun |
| 2 | 65–70 min | Resist early surge — no holeshot unless steep climb chaos repeats |
| 3–5 | 65–70 min | Race rhythm, protect these laps |
| 6–7 | 68–75 min | Accept fade, HR leads over power |
| 8 | Everything | Negative split goal |

---

## Pacing — Power & HR

### The One Rule
> **Never start a climb already in Z3.5. Come into climbs recovered.**

### Garmin 540 Screen Setup
- **Primary:** 3s power (large) | Lap avg power | HR zone | Lap timer
- **Secondary:** Course map
- Auto-lap: GPS trigger at start/finish coordinate

### By Terrain
| Terrain | Power | HR Zone | Notes |
|---------|-------|---------|-------|
| Sustained climb | 205–220W | Z3, cap Z3.9 | Z4 before lap 5 = back off |
| Punchy steep | ≤270W | Z4 briefly | Must return Z3 within 90 sec |
| Flat / rolling | 150–170W | Z2.5–Z3 | Z3.5 here = too hard |
| Descent / Vandelay | Soft pedal | Z2 or below | Recovery — not a time trial |

### Warning Signs
- Z4 on sustained climb before lap 5 → back off immediately
- HR not returning below Z3 within 90 sec after punch → ease next section
- Z3.5 on flat terrain → back off
- Still Z3 on descent → something is wrong upstream

---

## Nutrition Protocol

### Formula 369 + Maurten Gels
| Condition | Bottle | Scoops | Gel |
|-----------|--------|--------|-----|
| Normal | Large | 2 | Yes |
| Gut warning | Large | 2 | No |
| Gut distress | Plain water | — | No |
| Recovery lap | Resume normal | 2 | Yes |

- **Caffeine gels:** Max 3 total — laps 5–7 only
- **Gut management:** Act early at first signal. Plain water lap resets GI. Pass gas = resolved, resume normal next lap. Never push through distress.

---

## Pit Communication

### One-Word Signals (athlete → Renee)
| Signal | Renee Action |
|--------|-------------|
| **GOOD** | Large bottle 2sc + gel, send out |
| **ADJUST** | Large bottle 2sc, no gel |
| **WATER** | Plain water only, no gel |
| **LAYERS** | Add/remove layer per kit bag |
| **BATTERY** | Swap AXS derailleur battery |
| **MECH** | Tools out |

### Pit Rules
- Renee has next lap bottle ready before athlete arrives
- Renee does not ask questions unless athlete initiates
- Target pit time: under 90 seconds unless mechanical
- Renee may call one word back: **Easy** or **Good** — athlete's call to act on it
- Debrief after lap 4 only if needed — silence is good

### Pre-Race Bottle Prep (Renee, Friday night)
- Pre-mix all bottles, label by type: Large-2sc / Water
- Count gels — separate caffeine gels
- Inventory layer kit: wind vest, arm warmers, rain jacket, light gloves
- Confirm pit bag contents

---

## Layers Protocol
**Lesson from 2025: you froze when snow hit lap 5. Don't repeat.**

- Renee reads weather proactively — offers layer at next pit
- Put layers on one lap early. Remove one lap early.
- Don't wait until cold.

| Item | Deploy When |
|------|------------|
| Wind vest | Temperature drops, pre-storm sky |
| Arm warmers | Sustained cloud cover, cold descents |
| Rain jacket | Rain starting or imminent |
| Light gloves | Hands losing feel on descents |

---

## Mechanical Kit

### Renee's Pit Bag
- Spare tubes x2
- Tubeless plug kit + sealant top-up
- CO2 x3 + inflator
- SRAM Flattop quick links x2
- Multi-tool
- AXS spare battery (fully charged)
- Spare brake pads (MTX)
- Tire levers x2

### On-Bike
- Tubeless plugs
- CO2 x1
- Multi-tool
- Quick link x1

---

## Bike Setup
| Item | Spec |
|------|------|
| Front tire | Vittoria Barzo 29x2.4 TLR — 16psi |
| Rear tire | Vittoria Mezcal 29x2.4 TLR — 18psi |
| Fork | RockShox Pike Ultimate 130mm |
| Shock | RockShox Deluxe Ultimate |
| Drivetrain | SRAM XO Transmission |
| Brakes | SRAM Level — MTX ceramic pads |
| Chain | XO Flattop (new June 2026) |

---

## Key Reminders
1. **Lap 2** — controlled start, no holeshot unless the steep climb chaos is real
2. **Vandelay** — tight turns at end of lap when fatigued. Descent is recovery, not a time trial. Scout it Friday evening.
3. **Gut signal** — act at first sign, plain water, reset. Solved protocol from 2025.
4. **Layers** — one lap early beats freezing for three laps
5. **Lap 8** — everything left, negative split is the goal
6. **Pit time** — under 90 seconds. Renee runs the pit, trust her.

---

*Registration confirmed May 30, 2026 | Zone4 #7174873*
